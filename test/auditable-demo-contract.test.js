...
test("declaration binds the exact standalone binary without implicit authority", () => {
  ...
  assert.deepEqual(
    declaration.renditions.map(({ columns, rows, width, height }) => [
      columns,
      rows,
      width,
      height,
  ...
  assert.deepEqual(declaration.demos[0].steps[0].argv, [
    ...
    "./agent-hub-demo-run",
    "--output",
    "./agent-hub-demo-run/report.json",
    "--presentation",
  ]);
...
test("promotion materialization consumes the sole production Buildchain job", () => {
  ...
  assert.match(
    workflow,
    /auditable-demo-base-ref:[\s\S]*default: dev\/v0\/v0\.2/u,
  );
  assert.match(
    workflow,
  ...
  assert.match(
    workflow,
    /binary-artifact-name: \$\{\{ fromJSON\(needs\.auditable-demo-binary\.outputs\.artifact-coordinates-json\)\.artifacts\[0\]\.name \}\}/u,
  );
  assert.match(
    workflow,
    /binary-artifact-digest: \$\{\{ fromJSON\(needs\.auditable-demo-binary\.outputs\.artifact-coordinates-json\)\.artifacts\[0\]\.digest \}\}/u,
  );
  assert.match(workflow, /scenario-path: \.buildchain\/auditable-demo\.json/u);
  assert.match(
    workflow,
    /renderer-image: ghcr\.io\/kungfu-systems\/build-images\/demo-renderer@sha256:3a49708163fedaaabe07b45bba910026a1828151b5d4e9bbdaf0d62e75c927c1/u,
  );
  assert.doesNotMatch(
    workflow,
    /build\.yml@v3-alpha|buildchain-ref: v3-alpha/u,
  );
  assert.match(
    workflow,
    /auditable-demo-binary:[\s\S]*if: \$\{\{ github\.event_name == 'workflow_dispatch' && inputs\.auditable-demo-mode != 'off' \}\}/u,
  );
  assert.match(
    workflow,
    /auditable-demo-promotion:[\s\S]*needs: build[\s\S]*needs\.build\.outputs\.artifact-coordinates-json/u,
  );
  assert.doesNotMatch(
    workflow,
    /capture-auditable-demo|auditable-demo-passport|resolve-auditable-demo-source/u,
  );
});
...
test("binary metadata binds the exact executable closure", () => {
  for (const relative of [
    ...
    "scripts/verify-signed-binary.mjs",
  ]) {
    assert.match(
      read(relative),
      /executableFiles:\s*\[\{ path: [^,]+, sha256: [^}]+ \}\]/u,
      relative,
    );
  }
});
...
test("repository persists only reviewed floating Buildchain v4 authorities", () => {
  ...
  const retiredInvocation =
    /@kungfu-tech\/buildchain@3(?:\.|\b)|@v3(?:-alpha)?\b|Buildchain v3|"ref": "v3(?:-alpha)?"/u;
  ...
  for (const relative of workflowFiles) {
    const refs = [
      ...read(relative).matchAll(
        /kungfu-systems\/buildchain\/\.github\/workflows\/[^\s]+@([^\s]+)/gu,
      ),
    ];
    assert.ok(refs.length > 0, relative);
    assert.ok(
      refs.every((match) => ["v4", "v4-alpha"].includes(match[1])),
      relative,
    );
    assert.doesNotMatch(
      read(relative),
      /@[0-9a-f]{40}\b|buildchain-ref:\s*[0-9a-f]{40}\b/u,
      relative,
    );
  }
  ...
  const stableLock = JSON.parse(
    read(".buildchain/contract-lock.json"),
  ).buildchain;
  const alphaLock = JSON.parse(
    read(".buildchain/alpha-contract-lock.json"),
  ).buildchain;
  ...
  assert.equal(
    JSON.parse(read(".buildchain/platform-signing-policy.json"))
      .buildchainAuthority.exactSha,
    stableAuthority,
  );
});
