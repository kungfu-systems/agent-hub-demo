...
test("consumer signs Linux and macOS and declares Windows unsigned without credentials", () => {
  ...
  const policy = JSON.parse(
    fs.readFileSync(
      path.join(root, ".buildchain/platform-signing-policy.json"),
      "utf8",
    ),
  );
  ...
  assert.doesNotMatch(
    config,
    /certificate|password|private.?key|team.?id|notary|timestamp.?url|environment/iu,
  );
});

test("release verification consumes final platform bytes before KFD evidence", () => {
  const workflow = fs.readFileSync(
    path.join(root, ".github/workflows/artifact-signing-dogfood.yml"),
    "utf8",
  );
  const verifier = fs.readFileSync(
    path.join(root, "scripts/verify-signed-binary.mjs"),
    "utf8",
  );
  ...
  assert.doesNotMatch(
    workflow,
    /CERTIFICATE|PASSWORD|PRIVATE_KEY|TEAM_ID|NOTARY|TIMESTAMP_URL/,
  );
  assert.match(
    workflow,
    /artifact-finalization-command: npm run verify:final-artifact/,
  );
  assert.match(workflow, /artifact-finalization-on-platform: true/);
  assert.match(
    packageJson.scripts["verify:final-artifact"],
    /verify:signed-binary.*prepare:release-evidence/,
  );
